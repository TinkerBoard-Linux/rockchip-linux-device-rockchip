...
build_release()
{
	...
	RELEASE_BASE_DIR=$SDK_DIR/IMAGE
	...
	if yes | python3 .repo/repo/repo manifest -r \
		...
		mkdir -p "$PATCHES_DIR"
		python3 .repo/repo/repo forall -j $(( $CPUS + 1 )) -c \
	...
	if [ "$VERSION" == "release" ]; then
		mv $RELEASE_DIR/IMAGES/sdboot.img $RELEASE_DIR/$RELEASE_NAME.img
		zip -j -m -T $RELEASE_DIR/$RELEASE_NAME.zip $RELEASE_DIR/$RELEASE_NAME.img
		cd $RELEASE_DIR
		sha256sum $RELEASE_DIR/$RELEASE_NAME.zip > $RELEASE_DIR/$RELEASE_NAME.zip.sha256sum
		cd -
	fi
